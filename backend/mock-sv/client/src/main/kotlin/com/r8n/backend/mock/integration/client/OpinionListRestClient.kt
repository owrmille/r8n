...
class OpinionListRestClient(
    ...
    override fun getListSummary(listId: UUID): OpinionListSummaryDto =
        restClient.get().uri(SUMMARY_PATH, listId).retrieve().body<OpinionListSummaryDto>()!!

    override fun getList(listId: UUID): OpinionListDto =
        restClient.get().uri(GET_PATH, listId).retrieve().body<OpinionListDto>()!!

    override fun renameList(listId: UUID, name: String): OpinionListDto =
        restClient.patch().uri { uriBuilder ->
            uriBuilder.path(RENAME_PATH).queryParam("name", name).build(listId)
        }.retrieve().body<OpinionListDto>()!!

    override fun changePrivacy(listId: UUID, privacy: OpinionListPrivacyEnumDto): OpinionListDto =
        restClient.patch().uri { uriBuilder ->
            uriBuilder.path(SET_PRIVACY_PATH).queryParam("privacy", privacy).build(listId)
        }.retrieve().body<OpinionListDto>()!!

    override fun linkOpinion(listId: UUID, opinionId: UUID): OpinionListDto =
        restClient.post().uri { uriBuilder ->
            uriBuilder.path(LINK_PATH).queryParam("opinionId", opinionId).build(listId)
        }.retrieve().body<OpinionListDto>()!!

    override fun unlinkOpinion(listId: UUID, opinionId: UUID): OpinionListDto =
        restClient.patch().uri { uriBuilder ->
    ...
    override fun search(
        ...
        restClient.get().uri { uriBuilder ->
            uriBuilder.path(SEARCH_PATH)
                ...
                .build()
        }.retrieve().body<PageResponseDto<OpinionListSummaryDto>>()!!

    override fun syncWithOpinionList(existingListId: UUID, addedListId: UUID): OpinionListDto =
        restClient.post().uri { uriBuilder ->
            uriBuilder.path(SYNC_PATH).queryParam("addedListId", addedListId).build(existingListId)
        }.retrieve().body<OpinionListDto>()!!

    override fun unsyncWithOpinionList(existingListId: UUID, removedListId: UUID): OpinionListDto =
        restClient.post().uri { uriBuilder ->
