...
class RateLimitingFilter : AbstractGatewayFilterFactory<RateLimitingFilter.Config>(Config::class.java) {
    ...
    override fun apply(config: Config): GatewayFilter {
        return GatewayFilter { exchange: ServerWebExchange, chain: GatewayFilterChain ->
            val key = exchange.request.headers.getFirst("X-API-KEY") ?: exchange.request.remoteAddress?.address?.hostAddress ?: "unknown"
            val now = Instant.now()
            
            val counter = counters.compute(key) { _, current ->
