...
- Proxy: `/api` -> `https://localhost:8080` (gateway)
- No Nginx involved
...
Set the HTTPS host port in `deployment/config/docker.local.env`, for example `8443`.
...
- The current Docker configuration uses `INTERSERVICE_PROTOCOL=https`, so the gateway calls backend services over `https://opinions:8080` and `https://mock:8080`.
...
- `https://localhost:8080/` returns `404` (expected). The gateway is API-only.
...
- On rootless Docker, host ports `80` and `443` may fail to bind. Use an unprivileged host port (for example `8443`) by setting `FRONTEND_HTTPS_HOST_PORT` in `deployment/config/docker.local.env`
- `https://localhost:8080/` returns 404 -> expected; use real API routes
